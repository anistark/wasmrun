...
export function FunctionPlayground({ functions, onFunctionCall }: FunctionPlaygroundProps) {
  ...
  if (functions.length === 0) {
    return (
      <div class="text-center py-8 bg-light-surface2 dark:bg-dark-surface2 rounded-lg border border-light-surface3 dark:border-dark-surface3">
        <h4 class="text-lg font-medium text-light-textMuted dark:text-dark-textMuted mb-2">
  ...
  return (
    <div class="space-y-4">
      <div class="mb-6">
        <h3 class="text-xl font-medium text-light-textMuted dark:text-dark-textMuted mb-2">
          Function Playground
        </h3>
        <p class="text-sm text-light-textDim dark:text-dark-textDim">
          Interact with the WASM module's exported functions
        </p>
      </div>
      ...
      {functions.map(func => (
        <div
          key={func.name}
          class="bg-light-surface2 dark:bg-dark-surface2 rounded-lg p-4 border border-light-surface3 dark:border-dark-surface3"
        >
          <div class="mb-4 pb-2 border-b border-light-surface3 dark:border-dark-surface3">
            <h4 class="text-lg font-semibold text-light-accent2 dark:text-purple-400 font-mono mb-1">
              {func.name}
            </h4>
            ...
            {func.description && (
              <p class="text-sm text-light-textMuted dark:text-dark-textMuted">
                {func.description}
              </p>
            )}
          </div>
            ...
            {func.parameters.map(param => (
              <div key={param.name} class="space-y-1">
                <label class="text-sm font-medium text-light-warning dark:text-orange-300 font-mono">
                  {param.name}
                  <span class="text-xs text-light-textDim dark:text-dark-textDim italic ml-2">
                    ({param.type})
                  </span>
                </label>
                <input
            ...
            <div class="flex items-center gap-4 pt-3 border-t border-light-surface3 dark:border-dark-surface3">
              ...
              <div class="flex-1 px-3 py-2 bg-light-bg dark:bg-dark-bg border border-light-surface3 dark:border-dark-surface3 rounded font-mono text-sm min-h-8 flex items-center">
                {results[func.name] !== undefined ? (
                  ...
                  <span class="text-light-textDim dark:text-dark-textDim italic">
                    No result yet
                  </span>
                )}
              </div>
