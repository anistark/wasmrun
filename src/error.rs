...
mod tests {
    ...
    fn test_error_creation() {
        let error = ChakraError::file_not_found("/path/to/file.wasm");
        assert!(matches!(error, ChakraError::FileNotFound { .. }));
    }

    #[test]
    fn test_error_user_message() {
        let error = ChakraError::missing_tools(vec!["rustc".to_string()]);
        let message = error.user_message();
        assert!(message.contains("rustc"));
        assert!(message.contains("💡"));
    }

    #[test]
    fn test_error_suggestions() {
        let error = ChakraError::missing_tools(vec!["emcc".to_string()]);
        let suggestions = error.suggestions();
        assert!(!suggestions.is_empty());
        assert!(suggestions[0].contains("emcc"));
    }

    #[test]
    fn test_error_recoverable() {
        let non_recoverable = ChakraError::file_not_found("test.wasm");
    ...
    fn test_with_context() {
        ...
        let chakra_error = ChakraError::add_context("Reading configuration file", io_error);

        match chakra_error {
            ChakraError::WithContext { context, .. } => {
