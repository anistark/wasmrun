...
pub fn build_wasm(project_path: &str, output_dir: &str) -> Result<String, String> {
    println!("⚡️ Building WASM from AssemblyScript project at: {}", project_path);
    
    // Check if Node.js is installed
    let check_node = Command::new("node")
        .arg("--version")
        .output();
    ...
    let build_output = Command::new("npx")
        ...
        .args(["asc", "--optimize", "--outFile", "build/release.wasm", "assembly/index.ts"])
        .output();
    ...
    let wasm_file = if let Ok(output) = build_output {
        ...
        return Err("AssemblyScript compiler not found. Make sure it's installed in your project.".to_string());
    };
