...
pub fn build_wasm(project_path: &str, output_dir: &str) -> Result<String, String> {
    ...
    let check_tinygo = Command::new("tinygo")
        .arg("version")
        .output();
    
    if check_tinygo.is_err() {
        return Err("TinyGo is not installed or not in PATH. Please install TinyGo for WASM compilation.".to_string());
    }
    ...
    let entry_path = entry_file.ok_or_else(|| "No Go source files found in project directory".to_string())?;
    ...
    let output_name = entry_path.file_stem().unwrap().to_string_lossy().to_string() + ".wasm";
