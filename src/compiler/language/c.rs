...
pub fn build_wasm(project_path: &str, output_dir: &str) -> Result<String, String> {
    ...
    let check_emcc = Command::new("emcc")
        .arg("--version")
        .output();
    
    if check_emcc.is_err() {
        return Err("Emscripten (emcc) is not installed or not in PATH. Please install Emscripten.".to_string());
    }
    ...
    let entry_path = entry_file.ok_or_else(|| "No C source files found in project directory".to_string())?;
