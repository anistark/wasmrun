...
mod tests {
    ...
    #[test]
    fn test_github_registry() {
        let registry = GitHubRegistry::new("test-org".to_string());
    ...
    fn test_registry_manager() {
        ...
        let registries = manager.get_registries();

        assert!(registries.contains(&"crates.io"));
        assert!(registries.contains(&"github"));
    }

    #[test]
    fn test_search_filters() {
        let filters = SearchFilters {
            category: Some("development-tools".to_string()),
            min_downloads: Some(50),
        ...
        let entries = vec![
            ...
            RegistryEntry {
                ...
                downloads: 25, // Below min_downloads
                updated_at: 2000,
                registry_metadata: RegistryMetadata {
                    versions: vec!["1.0.0".to_string()],
                    documentation: None,
        ...
        // Only plugin1 should pass all filters
        assert_eq!(filtered.len(), 1);
        assert_eq!(filtered[0].info.name, "plugin1");
    }
    ...
    fn test_sort_by_name() {
        ...
        let entries = vec![
            RegistryEntry {
                info: PluginInfo {
                    ...
                    version: "1.0.0".to_string(),
                    description: "Last plugin".to_string(),
                    author: "Test".to_string(),
                    extensions: vec![],
                    entry_files: vec![],
                    plugin_type: PluginType::External,
        ...
        let sorted = apply_filters(entries, &filters);

        // Should be sorted alphabetically
        assert_eq!(sorted[0].info.name, "a-plugin");
        assert_eq!(sorted[1].info.name, "z-plugin");
    }
}
