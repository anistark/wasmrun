...
mod tests {
    ...
    #[test]
    fn test_verify_wasm_valid_file() {
        let temp_dir = tempdir().unwrap();
        let wasm_file = temp_dir.path().join("test.wasm");
        ...
        let result = verify_wasm(wasm_file.to_str().unwrap());
        assert!(result.is_ok());

        let verification = result.unwrap();
        assert!(verification.valid_magic);
        assert!(verification.section_count > 0);
    }

    #[test]
    fn test_verify_wasm_invalid_magic() {
        let temp_dir = tempdir().unwrap();
        let wasm_file = temp_dir.path().join("invalid.wasm");

        // Create file with invalid magic bytes
        fs::write(&wasm_file, b"invalid content").unwrap();
    ...
    fn test_is_entry_point() {
        ...
        assert!(!is_entry_point("random_function"));
