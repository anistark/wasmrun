...
pub fn verify_wasm(path: &str) -> Result<VerificationResult, String> {
...
fn read_leb128_u32(reader: &mut Cursor<Vec<u8>>) -> Result<u32, String> {
    let mut result = 0u32;
    let mut shift = 0;
...
pub fn print_detailed_binary_info(path: &str) -> Result<(), String> {
    // Read file
    let wasm_bytes = fs::read(path).map_err(|e| format!("Error reading file: {}", e))?;
